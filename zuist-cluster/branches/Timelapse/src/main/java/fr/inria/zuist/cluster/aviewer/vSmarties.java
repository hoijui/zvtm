...
//SmartiesWidget toggle_play;
int ui_toggle_play;
int countWidget = 0;

SmartiesDevice device;

public void playOff(){
	m_smarties.sendWidgetLabel(ui_toggle_play, "Play: Off", device);
    m_smarties.sendWidgetOnState(ui_toggle_play, false, device);
}

public void playCount(int c){
	m_smarties.sendWidgetLabel(ui_toggle_play, "Play: "+c, device);
}
...
vSmarties(Viewer v)
{
	...
	/*
	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_BUTTON, "Reset View", 1, 1, 2, 1);
	...
	*/
	...
	w.handler = new rightClicked();//leftClicked();
	countWidget++;
	
	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_BUTTON, ">", 2, 1, 1, 1);
	w.handler = new leftClicked();
	countWidget++;
	

	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_BUTTON, "+", 1, 2, 1, 1);
	w.handler = new zoomInClicked();
	countWidget++;

	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_BUTTON, "-", 2, 2, 1, 1);
	w.handler = new zoomOutClicked();
	countWidget++;

	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_TOGGLE_BUTTON, "Play: Off", 3, 1, 1, 2);
		//SmartiesWidget.SMARTIES_WIDGET_TYPE_BUTTON, "Play", 3, 1, 1, 2);
    w.labelOn = new String("Play: On");
	w.handler = new autoClicked();
	ui_toggle_play = countWidget;
	countWidget++;


	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_BUTTON, "Reset View", 4, 1, 1, 2);
	w.handler = new globalViewClicked();
	countWidget++;

	/*
	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_TOGGLE_BUTTON, "Draw Under Bezel: Off", 3, 1, 2, 2);
	...
	*/
	...
	w = m_smarties.addWidget(
		SmartiesWidget.SMARTIES_WIDGET_TYPE_SPINNER, "Open Images", 5, 1, 2, 2);
	...
	w.items.add("eso");
	w.items.add("VISTA milky way");
	w.items.add("GLIMPSE milky way");
	w.items.add("France");
	w.items.add("Santiago");
	w.items.add("IGN");
	w.items.add("BMNG");
	...
	//xmlfiles.add("/usr/local/share/bigimages/zuist/NASA-Curiosity/ZVTM/scene.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/misc/paris26GP/scene_fullL0.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/astro/eso/scene.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/astro/eso1242a/scene.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/astro/ssc2008-11/scene.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/geo/osm/france_bright/scene.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/geo/osm/adkintun/scene.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/geo/ign/Grenoble/scene.xml");
 	xmlfiles.add("/home/wall/zuist-scenes/geo/bmng/200410/_scene.xml");
...
public class autoClicked implements SmartiesWidgetHandler
{
	public boolean callback(SmartiesWidget w, SmartiesEvent e, Object user_data)
	{
		device = e.device;
...
public class leftClicked implements SmartiesWidgetHandler
{
	public boolean callback(SmartiesWidget w, SmartiesEvent e, Object user_data)
	{
		
		viewer.translateView(Viewer.MOVE_SLOW_LEFT);
		viewer.option = Viewer.MOVE_SLOW_LEFT;
		return true;
	}
}

public class rightClicked implements SmartiesWidgetHandler
{
	public boolean callback(SmartiesWidget w, SmartiesEvent e, Object user_data)
	{
		
		viewer.translateView(Viewer.MOVE_SLOW_RIGHT);
		viewer.option = Viewer.MOVE_SLOW_RIGHT;
		return true;
	}
}

public class zoomInClicked implements SmartiesWidgetHandler
{
	public boolean callback(SmartiesWidget w, SmartiesEvent e, Object user_data)
	{
		double x = 0.5;
		double y = 0.5;
		if(e != null && e.p != null && e.p.app_data != null){
			myCursor c = (myCursor)e.p.app_data;
...
public class zoomOutClicked implements SmartiesWidgetHandler
{
	public boolean callback(SmartiesWidget w, SmartiesEvent e, Object user_data)
	{
		double x = 0.5;
