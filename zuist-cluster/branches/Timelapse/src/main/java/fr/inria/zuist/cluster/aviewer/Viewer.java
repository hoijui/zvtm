...
public class Viewer implements Java2DPainter, RegionListener, LevelListener {
	...
	static final float REFRESH_PERIOD = 5.9f;
    ...
    static final short MOVE_SLOW_UP = 4;
    static final short MOVE_SLOW_DOWN = 5;
    static final short MOVE_SLOW_LEFT = 6;
    static final short MOVE_SLOW_RIGHT = 7;
    static final short ZOOM_IN = 8;
    static final short ZOOM_OUT = 9;
    ...
    void translateView(short direction){
        ...
        else if(direction==MOVE_SLOW_UP){
        	double qt = (rb[1]-rb[3])/128.0;
            trans = new Point2D.Double(0,qt);
        }
        else if(direction==MOVE_SLOW_DOWN){
        	double qt = (rb[3]-rb[1])/128.0;
            trans = new Point2D.Double(0,qt);
        }
        else if(direction==MOVE_SLOW_RIGHT){
        	double qt = (rb[2]-rb[0])/128.0;
            trans = new Point2D.Double(qt,0);
        }
        else{// if(direction==MOVE_SLOW_LEFT){
        	double qt = (rb[0]-rb[2])/128.0;
            trans = new Point2D.Double(qt,0);
        }
    ...
    public void playAnimation(){
    	...
    	if(!play){
    		...
    		timer = new Timer();
        	period = (long)(1000 * REFRESH_PERIOD);

        	timer.schedule(new UpdateAnimation(), period, period);
        	play = true;
    	} else {
    ...
    class UpdateAnimation extends TimerTask{
	    ...
	    public void run(){
	        ...
	        if(count < max){
	        	...
	        	if(option >= Viewer.MOVE_UP && option <= Viewer.MOVE_SLOW_RIGHT){
	        		translateView(option);
	        	} else {
	        		centredZoom(f, x, y);
	        	}
	        	vs.playCount(count);
