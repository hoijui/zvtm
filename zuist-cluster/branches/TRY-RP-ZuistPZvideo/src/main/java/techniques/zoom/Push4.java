...
public class Push4 extends AbstractZoomTechnique {
	...
	public static final int MAX_MVT = 100;
	...
	public Push4(String id, ORDER o, int portIn, boolean markersOnBack) {
				...
				if (msg != null && msg.getAddress().equals(Zoom.MOVE_OBJECT)) {
					...
					if (parts != null && parts.length == 6) {
						...
						if (previousX == Integer.MAX_VALUE && previousY == Integer.MAX_VALUE && previousZ == Integer.MAX_VALUE) {
							...
							if (pressed && Math.abs(movementY) < MAX_MVT) {
							
								// Right yaw value for zooming in
								if ( dy > 0 ) {
									
									// Right movement amplitude and direction
									if (movementY > MIN_MVT) { // TODO to be tested
