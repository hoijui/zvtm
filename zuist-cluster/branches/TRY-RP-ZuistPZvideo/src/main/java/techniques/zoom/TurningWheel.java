...
public class TurningWheel extends AbstractZoomTechnique {
	...
	public static final int MIN_PASS_VALUE_ONEHANDED = 30;
	...
	public TurningWheel(String id, ORDER o, int portIn) {
		
		super(id, o, false);
		
		System.out.println("new TurningWheel()");
		...
		laserListener = new VICONLaserListener() {
			...
			public void acceptMessage(Date time, OSCMessage msg) {
				if (msg != null) 
				{
					...
					String cmd = (String)parts[3];
					max_distance = MAX_DISTANCE_ONEHANDED;
					min_pass_value = MIN_PASS_VALUE_ONEHANDED;

					if (parts != null && parts.length == 4 && cmd.equals(IN_CMD_TURNING_WHEELONSCREEN))
					{
							super.acceptMessage(time, msg);
							
							if(zoomMouseEvent){
								Long Nx = new Long(currentCoords.x);
								Long Ny = new Long(currentCoords.y);
								
								System.out.println("Calling TurningWheel()");
								
								turningWheel(Nx.floatValue(), Ny.floatValue());
							}
						}
					}
				}
...
//				System.out.println("\n(" + IN_CMD_TURNING_WHEEL + ")");
				...
				if (cmd.equalsIgnoreCase(IN_CMD_TURNING_WHEEL)) {
		...
		dataReceiver.addListener("/WildPointing/moveTo", laserListener);
		// dataReceiver.addListener("/WildPointing/moveTo", VICONListener);
