...
public class VICONLaserListener implements OSCListener {
	...
	protected Point2D.Double currentCoords = new Point2D.Double(0, 0);
	protected Point2D.Double previousCoords = new Point2D.Double(0, 0);
	...
	public void acceptMessage(Date time, OSCMessage message) {
		...
		if (message != null && message.getArguments() != null && message.getArguments().length > 2) {
			...
			if (!quit) {
				...
				currentCoords = new Point2D.Double(screenCoordsX * screenP.width + screenX, screenCoordsY * screenP.height + screenY);
	...
	public Point2D.Double getCurrentCoords() {
