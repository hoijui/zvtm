...
SmartiesPuck pitchPuck = null;
SmartiesDevice pitchDevice = null;
SmartiesPuck dragPuck = null;
SmartiesDevice dragDevice = null;

float prevMFPitchD = 0f;
...
public void update(Observable obj, Object arg)
{
	...
	{
		//System.out.println("Start MF Pitch: " + e.id);
		if (pitchDevice == null)
		{
			// zoom not locked by an other device
			pitchDevice = e.device;
			pitchPuck = e.p;
			prevMFPitchD = e.d;
		}
		break;
	...
	{
		//System.out.println("MF Pitch: " + e.id);
		if (pitchDevice == e.device)
		{
			// this is the device that lock the zoom e.p shoul be == pitchPuck
			// TODO: zoom centred on pitchPuck (if null center on the center of the pitch)
			if (e.d != 0)
			{
				double x = e.x;
				double y = e.y;
	...
	{
		//System.out.println("End MF Pitch: " + e.id);
		if (pitchDevice == e.device)
		{
