...
class GResize extends GAnimation{
    /**step values for size*/
    float[] steps;
     ...
     */
    GResize(Glyph g,AnimManager mgr,long d){
	started=false;
	target=g;	
	parent=mgr;
	duration=d;
	type=AnimManager.GL_SZ;	
    }
	...
	if (started){
		...
		target.sizeTo(steps[step]);
	    }
	    else {
		target.sizeTo(steps[steps.length-1]);
	...
	if (paa != null){
	    paa.animationEnded(target, PostAnimationAction.GLYPH, AnimManager.GL_SZ);
	}
    }
