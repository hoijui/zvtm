...
// 	    vsm.addGlyph(c, mSpace);
	    ...
	    try {
		SVGReader.setPositionOffset(-7150,-4400);
		...
		vsm.addGlyph(new VRectangle(Distractors.GRAPH_DISTRACTOR_NODES[i].x, Distractors.GRAPH_DISTRACTOR_NODES[i].y, 0, 5, 5, DISTRACTOR_COLOR), mSpace);
	    }
	    for (int i=0;i<Distractors.GRAPH_DISTRACTOR_EDGES_HEAD.length;i++){
		vsm.addGlyph(new VSegment(Distractors.GRAPH_DISTRACTOR_EDGES_TAIL[i].x, Distractors.GRAPH_DISTRACTOR_EDGES_TAIL[i].y,
					  0, Color.BLACK,
					  Distractors.GRAPH_DISTRACTOR_EDGES_HEAD[i].x, Distractors.GRAPH_DISTRACTOR_EDGES_HEAD[i].y),
			     mSpace);
	    }
	    // potential targets
	    // potential targets R1
