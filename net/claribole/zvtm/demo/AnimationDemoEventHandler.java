...
class AnimationDemoEventHandler implements ViewEventHandler {
	...
	if (buttonNumber == 3 || ((mod == META_MOD || mod == META_SHIFT_MOD) && buttonNumber == 1)){
		...
		application.vsm.animator.Xspeed = 0;
		application.vsm.animator.Yspeed = 0;
		application.vsm.animator.Aspeed = (c.altitude>0) ? (long)((lastJPY-jpy) * (a/SPEED_FACTOR)) : (long)((lastJPY-jpy) / (a*SPEED_FACTOR));
	    }
	    else {
		application.vsm.animator.Xspeed = (c.altitude>0) ? (long)((jpx-lastJPX) * (a/SPEED_FACTOR)) : (long)((jpx-lastJPX) / (a*SPEED_FACTOR));
		application.vsm.animator.Yspeed = (c.altitude>0) ? (long)((lastJPY-jpy) * (a/SPEED_FACTOR)) : (long)((lastJPY-jpy) / (a*SPEED_FACTOR));
		application.vsm.animator.Aspeed = 0;
	    }
	}
    }
