...
class AbstractWorldManager {
    ...
    static int TREE_DEPTH = 5; // we want subjects to navigate through 5 levels, includes root
    static final long SMALLEST_ELEMENT_WIDTH = 50;
    static final long MUL_FACTOR = 100;
	...
	root.setTarget(new VRoundRect(0, 0, 0, widthByLevel[0], widthByLevel[0]/2, colorByLevel[0], (int)widthByLevel[0]/10, (int)widthByLevel[0]/10));
	...
	for (int i=0;i<NB_CHILDREN-1;i++){
	    vx = Math.round(ar.target.vx + Math.random()*ar.target.getWidth()*2-ar.target.getWidth());
	    vy = Math.round(ar.target.vy + Math.random()*ar.target.getHeight()*2-ar.target.getHeight());
	    ds[i] = new VRectangle(vx, vy, 0, widthByLevel[depth], widthByLevel[depth]/2, colorByLevel[depth]);
	...
	vx = Math.round(ar.target.vx + Math.random()*ar.target.getWidth()*2-ar.target.getWidth());
	vy = Math.round(ar.target.vy + Math.random()*ar.target.getHeight()*2-ar.target.getHeight());
 	VRoundRect tr = new VRoundRect(vx, vy, 0, widthByLevel[depth], widthByLevel[depth]/2, colorByLevel[depth], (int)widthByLevel[depth]/10, (int)widthByLevel[depth]/10);
