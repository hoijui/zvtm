...
public class VCircleST extends VCircle implements Transparent,Cloneable {
	...
	if (pc[i].cr>1){
	    ...
	    g.setComposite(acST);
	    if (filled){g.fillOval(pc[i].cx-pc[i].cr,pc[i].cy-pc[i].cr,2*pc[i].cr,2*pc[i].cr);}
	    g.setComposite(acO);
	...
	if (pc[i].lcr>1){
	    g.setColor(this.color);  
	    g.setComposite(acST);
	    if (filled){g.fillOval(pc[i].lcx-pc[i].lcr,pc[i].lcy-pc[i].lcr,2*pc[i].lcr,2*pc[i].lcr);}
	    g.setComposite(acO);
	    g.setColor(borderColor);
	    if (paintBorder){
		if (stroke!=null) {
