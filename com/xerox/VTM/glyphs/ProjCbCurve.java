/*   FILE: ProjCbCurve.java
 *   DATE OF CREATION:   Oct 03 2001
 *   AUTHOR :            Emmanuel Pietriga (emmanuel.pietriga@xrce.xerox.com)
 *   MODIF:              Thu Jan 24 10:29:57 2002 by Emmanuel Pietriga
 *   Copyright (c) Xerox Corporation, XRCE/Contextual Computing, 2002. All Rights Reserved
 *
 * This library is free software; you can redistribute it and/or
 * modify it under the terms of the GNU Lesser General Public
 * License as published by the Free Software Foundation; either
 * version 2.1 of the License, or (at your option) any later version.
 * 
 * This library is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
 * Lesser General Public License for more details.
 *
 * For full terms see the file COPYING.
 *
 * $Id: ProjCbCurve.java,v 1.4 2005/12/08 09:08:21 epietrig Exp $
 */

package com.xerox.VTM.glyphs;
import java.awt.geom.CubicCurve2D;
import java.awt.geom.Point2D;

/**project coordinates of a quadratic curve
 * @author Emmanuel Pietriga
 */

class ProjCbCurve extends BProjectedCoords {

    /**curve*/
    CubicCurve2D.Double quad=new CubicCurve2D.Double();
    /**start point*/
    Point2D.Double start=new Point2D.Double();
    /**end point*/
    Point2D.Double end=new Point2D.Double();
    /**control point 1 (controls start point tangent vector)*/
    Point2D.Double ctrlStart=new Point2D.Double();
    /**control point 2 (controls end point tangent vector)*/
    Point2D.Double ctrlEnd=new Point2D.Double();

    /**curve*/
    CubicCurve2D.Double lquad=new CubicCurve2D.Double();
    /**start point*/
    Point2D.Double lstart=new Point2D.Double();
    /**end point*/
    Point2D.Double lend=new Point2D.Double();
    /**control point 1 (controls start point tangent vector)*/
    Point2D.Double lctrlStart=new Point2D.Double();
    /**control point 2 (controls end point tangent vector)*/
    Point2D.Double lctrlEnd=new Point2D.Double();

}
