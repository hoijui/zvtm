 ...
 *
 * $Id: ZRoundRect.java 1032 2007-12-10 10:15:08Z epietrig $
 */
...
public class ZRoundRect extends VRoundRect {
	...
	if (renderRound){
		...
		if (isBorderDrawn() && (pc[i].aw > ROUND_CORNER_THRESHOLD || pc[i].ah > ROUND_CORNER_THRESHOLD)){
	...
	if (renderRound){
		...
		if (isBorderDrawn() && (pc[i].law > ROUND_CORNER_THRESHOLD || pc[i].lah > ROUND_CORNER_THRESHOLD)){
		    g.setColor(borderColor);
