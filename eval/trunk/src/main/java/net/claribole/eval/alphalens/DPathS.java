...
public class DPathS extends Glyph implements RectangularShape {
	...
	Stroke lensStroke = new BasicStroke(1.0f);
     ...
     *@param c color
     */
    public DPathS(PathIterator pi, int z, Color c, float lensStrokeWidth){
		vz = z;
	...
	if (pc != null){
	    ...
	    else {System.err.println("DPathS:Error while adding camera "+verifIndex);}
	}
	else {
	    if (verifIndex == 0){
	...
	if (stroke!=null) {
		...
		if (elements[j].type == DPathS.MOV){continue;}
	...
	/**
		* Edit coordinates of start, end and control points of the element in DPathS
		* @param index index of the element in the DPathS
	...
	/**
		* Transform DPathS by translating each of the points
	...
	public void edit(LongPoint[] points, boolean abs){
		...
		for (int i=0; i < elements.length; i++){
			...
			switch (elements[i].type){
				...
				case DPathS.CBC:{totalPointsCount += 2; break;}
				// One additional point
				case DPathS.QDC:{totalPointsCount += 1; break;}
			}
		}
     ...
     * Get element's type
     * @param index index of the element in the DPathS
     * @return -1 in case of incorrect parameters, otherwise returns one of the constants DPathS.CBC, DPathS.QDC, DPathS.SEG, DPathS.MOV 
     */
	...
	public int getNumberOfPoints(){
		...
		for (int i=0; i < elements.length; i++){
		    ...
		    switch (type){
		    case DPathS.CBC:{totalNumberOfPoints += 2; break;}
		    case DPathS.QDC:{totalNumberOfPoints += 1; break;}
		    }
		}
    ...
    /**
     * Calculates coordinates of all DPathS's points (including control points) to display the DPathS as a line.
     * @param path DPathS to be flatten
     ...
     */
    public static LongPoint[] getFlattenedCoordinates(DPathS path, LongPoint startPoint, LongPoint endPoint, boolean abs){
	...
	/** Get an SVG-compatible path iterator for this DPathS. */
	...
	/** Get a Java2D path iterator for this DPathS. */
    public PathIterator getJava2DPathSIterator(){
		GeneralPath res = new GeneralPath();
		res.moveTo(spx, spy);
		for (int i = 0; i < this.getElementsCount(); i++){
			int elType = this.getElementType(i);
	...
	/** Get a Java2D GeneralPath iterator for this DPathS. */
