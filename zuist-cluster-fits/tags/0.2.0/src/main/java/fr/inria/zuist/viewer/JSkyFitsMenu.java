...
public class JSkyFitsMenu implements ViewListener{
	...
	////"Background", "Blue", "Heat", "Isophot", "Light", "Pastel", "Ramp", "Real", "Smooth", "Staircase", "Standard"
	/*
    ...
    */
    ...
    public static final ColorGradient[] COLOR_GRADIANT = {
    	new RampFilter(), new HeatFilter(), new BlulutFilter(), new Idl12Filter(), new Rainbow1Filter(), new StandardFilter(), new Idl4Filter(), new Stairs8Filter()
    };

    public static final String[] COLOR_NAME = {
    	"Ramp", "Heat", "Blulut", "Idl12", "Rainbow1", "Standard", "Idl4", "Stairs8"
    };
...
//	int CONST = 636;
	...
	private void drawFiltersColor(){
		...
		//BORDER_BOTTON_FILTER = app.VIEW_H/2 - (COLOR_GRADIANT.length * ( HEIGHT_BTN + BORDER*2 ) + SCALE_METHOD.length * ( HEIGHT_BTN + BORDER*2 ) + BORDER_TOP_FILTER) + 2*HEIGHT_BTN + BORDER;
		BORDER_BOTTON_FILTER = app.VIEW_H/2 - py - (HEIGHT_BTN + 2*BORDER);
	}
	...
	public void press1(ViewPanel v, int mod, int jpx, int jpy, MouseEvent e){
		if(jpy > app.menu.BORDER_TOP_HISTOGRAM && jpy < app.menu.BORDER_BOTTON_HISTOGRAM){
			lastJPX = jpx;
			if(shadow != null)
				app.menuSpace.removeGlyph(shadow);

			press1 = true;

			//Point2D.Double point = app.viewToSpace(app.menuCamera, jpx, jpy);
	...
	public void mouseMoved(ViewPanel v,int jpx,int jpy, MouseEvent e){
        ...
        if((jpx < app.menu.WIDTH_MENU && jpy < app.menu.BORDER_BOTTON_FILTER && jpy > app.menu.BORDER_TOP_FILTER) ||
          (jpy > app.menu.BORDER_TOP_HISTOGRAM && jpy < app.menu.BORDER_BOTTON_HISTOGRAM && jpx > app.menu.BORDER_LEFT_HISTOGRAM && jpx < app.menu.BORDER_RIGHT_HISTOGRAM )){
        	...
        	if(v.parent.getActiveLayer() != app.LAYER_SCENE){
	...
	public void enterGlyph(Glyph g){
        ...
        if(g.getType().equals(T_FILTER)){
        	...
        	if(color_selected != ((PRectangle)g)){
        		...
        		app.setColorFilter((String)g.getOwner());
        	...
        	if(scale_selected != (PRectangle)g){
        		...
        		app.setScaleMethod((JSkyFitsImage.ScaleAlgorithm)g.getOwner());
