...
do
  for row in {1..4}
    do
      ...
      ssh wall@$col$row.wall.inria.cl -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no "export DISPLAY=:0.0 && cd /home/wall/Andes/zuist-cluster-fits/trunk && java -XX:+DoEscapeAnalysis -XX:+UseConcMarkSweepGC -Djava.net.preferIPv4Stack=true -Djgroups.bind_addr="\"$col$row.wall.inria.cl\"" -Xmx4096M -Xms2048M -Dcom.sun.media.jai.disableMediaLib=true -cp $JARS fr.inria.zvtm.cluster.SlaveApp -n JSkyFitsViewer -b $SLAVENUM1 -f" $* &
      sleep 1
      ssh wall@$col$row.wall.inria.cl -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no "export DISPLAY=:0.1 && cd /home/wall/Andes/zuist-cluster-fits/trunk && java -XX:+DoEscapeAnalysis -XX:+UseConcMarkSweepGC -Djava.net.preferIPv4Stack=true -Djgroups.bind_addr="\"$col$row.wall.inria.cl\"" -Xmx4096M -Xms2048M -Dcom.sun.media.jai.disableMediaLib=true -cp $JARS fr.inria.zvtm.cluster.SlaveApp -n JSkyFitsViewer -b $SLAVENUM2 -f" $* &
    done
done
