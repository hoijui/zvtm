...
public class DragMag extends DraggableCameraPortal implements ComponentListener, CameraListener {
    ...
    public DragMag(int x, int y, int width, int height,  Camera c, Camera extc, VirtualSpace gs, double scaleFactor, int type)
    {
        ...
        this.saveDragBarHeight = getDragBarHeight();
    ...
    public void setType(int t)
    {
        ...
        if (type == DM_TYPE_MANATTHAN){
            type = t;
            setDragBarHeight(saveDragBarHeight);
            updateVis();
            return;
        }
        ...
        if (t == DM_TYPE_MANATTHAN){
            saveDragBarHeight = getDragBarHeight();
            setDragBarHeight(0);
            type = t;
