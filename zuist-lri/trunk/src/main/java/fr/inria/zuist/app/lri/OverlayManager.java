...
/*    static final Font SMALL_FONT = new Font("Arial", Font.PLAIN, 8);
    static final Font NORMAL_FONT = new Font("Arial", Font.PLAIN, 12);
*/
    ...
    OverlayManager(LRIExplorer app){
    ...
    void displayLinksP1(String[] labels, String[] IDs, String titleS){
        ...
        for (int i=0;i<links.length;i++){
            VTextST t = new VTextST(0, 0, 0, (IDs[i] != null) ? LINK_COLOR : DISABLED_LINK_COLOR,
                labels[i], VText.TEXT_ANCHOR_START, 0, (links.length < MAX_NB_ITEMS_FOR_NORMAL_FONT) ? NORMAL_FONT_SCALE_FACTOR: SMALL_FONT_SCALE_FACTOR);
            t.setType(IDs[i]);
    ...
    void waitForBounds(final LongPoint[] linkBounds, final LongPoint[] titleBounds){
        // going through this is actually required because for some unknown reason,
        // a single call to repaintNow() before going from displayLinksP1() to displayLinksP2()
        // is not sufficient on some occasions (seems completely random, though I know it is not)
        // the basic idea here "repaint view until the bounds of all VText objects have actually been computed"
    ...
    void displayLinksP2(LongPoint[] linkBounds, LongPoint[] titleBounds){ 
        ...
        for (int i=0;i<links.length;i++){
            ...
            application.vsm.animator.createGlyphAnimation(LRIExplorer.ANIM_MOV_LENGTH, AnimManager.GL_COLOR_LIN, LINK_ANIM_APPEAR, links[i].getID());
            application.vsm.animator.createGlyphAnimation(LRIExplorer.ANIM_MOV_LENGTH, AnimManager.GL_TRANS_SIG, transData[i], links[i].getID());
        }
        // div by 2 to make it fit perectly (no padding)
    ...
    void hideLinks(boolean animate){
        if (animate){
            for (int i=0;i<links.length;i++){
                application.vsm.animator.createGlyphAnimation(LRIExplorer.ANIM_MOV_LENGTH, AnimManager.GL_COLOR_LIN, LINK_ANIM_DISAPPEAR,
                                                  links[i].getID(), new GlyphKillAction(application.vsm));                
            }
        }
        else {
            for (int i=0;i<links.length;i++){
