...
class ExplorerEventHandler implements ViewEventHandler, AnimationListener, ComponentListener {
    ...
    public void press1(ViewPanel v,int mod,int jpx,int jpy, MouseEvent e){
        if (application.NAV_MODE == LRIExplorer.NAV_MODE_SS){
    ...
    public void mouseMoved(ViewPanel v,int jpx,int jpy, MouseEvent e){
        if (LRIExplorer.SHOW_STATUS_BAR){
        ...
        currentJPX = jpx;
        currentJPY = jpy;
        if ((jpx-LRIExplorer.LENS_R1) < 0){
            jpx = LRIExplorer.LENS_R1;
            cursorNearBorder = true;
        }
        else if ((jpx+LRIExplorer.LENS_R1) > application.panelWidth){
            jpx = application.panelWidth - LRIExplorer.LENS_R1;
            cursorNearBorder = true;
        }
        else {
            cursorNearBorder = false;
        }
        if (jpy < 2*LRIExplorer.LENS_R1){
            jpy = 2*LRIExplorer.LENS_R1;
            cursorNearBorder = true;
        }
       ...
       /*else if ((jpy) > application.panelHeight){
            jpy = application.panelHeight - LRIExplorer.LENS_R1;
            cursorNearBorder = true;
        }*/
        lastJPX = jpx;
        lastJPY = jpy;
