...
cam.setAltitude(150.0f)

// relative altitude change
cam.altitudeOffset(-10.0f);
...
\begin{small}
\begin{SaveVerbatim}{CodeVerb}
cam.setLocation(new Location(100,-200,150f));
\end{SaveVerbatim}
\fbox{\BUseVerbatim[boxwidth=0.99\columnwidth]{CodeVerb}}
\end{small}

A \cd{fr.inria.zvtm.engine.CameraListener} can be associated with a \cd{Camera} to be notified about camera movements (both (x,y) and altitude changes).
...
In many applications the camera's position and altitude is controlled by pan \& zoom actions initiated by the user through an input device such as a mouse, trackpad, trackball. This is easily implemented in ZVTM with a few lines of code in the application's \cd{ViewEventHandler} (see Section \ref{sec:input} for more detail about this interface).
...
public void mouseDragged(ViewPanel v,int mod,int buttonNumber,int jpx,int jpy,
    ...
    if (buttonNumber == 3){
        Camera c = VirtualSpaceManager.INSTANCE.getActiveCamera();
        VirtualSpaceManager.INSTANCE.getAnimationManager().setXspeed(
            (c.altitude>0) ? (long)((jpx-lastJPX)*(a/RBSSF))
                           : (long)((jpx-lastJPX)/(a*RBSSF)));
        VirtualSpaceManager.INSTANCE.getAnimationManager().setYspeed(
            (c.altitude>0) ? (long)((lastJPY-jpy)*(a/RBSSF))
