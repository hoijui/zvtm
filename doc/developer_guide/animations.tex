...
below) and of controlling animations ("normal" animations and the current camera 
animation). 
...
the \cd{begin}, \cd{end} and \cd{repeat} methods, leaving only the 
\cd{timingEvent} method to implement.
...
when it will be replaced by the next animation in line, if it exists.
...
Parallel to the animation mechanism described above, the Animation Manager has a 
...
package \cd{net.claribole.zvtm.animation.examples}) implements it.
