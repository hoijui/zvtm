...
zvtm-cluster can use the Agile2d OpenGL rendering pipeline through zvtm-agile2d. To build
...
\begin{itemize}
\item
  At the very beginning of your program, call \cd{VirtualSpaceManager.INSTANCE.setMaster()}
\item
  Create one or more \cd{ClusteredView} instances
\end{itemize}

Examples are located in the \cd{src/main/java/fr/inria/zvtm/cluster/examples}. Studying the
...
\subsection{About displaying images with zvtm-cluster}
...
\textbf{Note about OpenGL acceleration: } for OpenGL acceleration to be
enabled through the Sun Java2D OGL rendering pipeline,
you must define sun.java2d.opengl in addition to passing the
...
by the zvtm-agile2D (JOGL-based) views. The latter works on any platform, including Mac OS X, and does not require this property to be set to true.
