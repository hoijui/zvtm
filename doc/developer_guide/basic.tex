...
If ZVTM is instantiated within an applet, the VSM should be notified about this:

\codebox{vsm.setApplet();}
...
There are three main types of views \cl{com.xerox.VTM.engine.View}: external views \cl{EView, GLEView, AccEView}, which are managed by the windowing system, e.g., Mac OS X, Windows, X11/Xorg; panel views \cl{PView}\footnote{Formerly miscalled AppletView because it was used mostly within a Swing JApplet.}, which can be inserted in any Swing hierarchy; and internal views, which are put on a Java JDesktopPane \cl{IView,AccIView}.
...
Views with prefix \cd{Acc} are based on Java \cd{VolatileImage}. Other views (without a prefix) use standard \cd{BufferedImage}s to draw their content offscreen and the copy it to the display. The latter are the only views that support magnification lenses (Section \ref{sec:lenses}).
...
cameras.add(foocam);
View v = vsm.addExternalView(cameras, "fooview", View.STD_VIEW, 800, 600, true, true);
\end{SaveVerbatim}
\fbox{\BUseVerbatim[boxwidth=0.99\columnwidth]{CodeVerb}}
\end{small}
...
\fbox{JPanel p = vsm.addExternalView(cameras, "fooView", 800, 600);}
...
cameras.add(barcam);
cameras.add(lastcam);
View v = vsm.addExternalView(cameras, "3_layer_view", View.STD_VIEW, 800, 600, true, true);
\end{SaveVerbatim}
\fbox{\BUseVerbatim[boxwidth=0.99\columnwidth]{CodeVerb}}
\end{small}
...
%%%%%
\subsubsection{Drawing stack and Glyph z-index, and Java2D painters}
...
ZVTM also provides developers with convenient callback methods for calling raw Java2D instructions at various stages of the rendering process, through the \cl{net.claribole.zvtm.engine.Java2DPainter} interface. This can be useful, e.g., for drawing a static label in a corner of the view, a logo or a memory  gauge (see Figure \ref{fig:j2dp}).
...
cams.add(detailCam);
View v = vsm.addExternalView(cams, ...);
...
It is possible to associated the same event handler with all layers in a view, or to associate different event handlers with each layer of the view. The input event handling interface works in a way similar to conventional AWT/Swing event listeners (using callbacks). There is a single interface for all ZVTM events: \cl{net.claribole.zvtm.engine.ViewEventHandler}. A convenience (default) implementation, which does nothing, is available for subclassing: \cl{DefaultEventHandler}.
...
Portals have their own event interface \cl{net.claribole.zvtm.engine.PortalEventHandler}, that is used to listen for cursor entry/exit.
