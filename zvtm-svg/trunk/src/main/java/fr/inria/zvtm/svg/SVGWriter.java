...
public class SVGWriter {
    ...
    public Document exportVirtualSpace(VirtualSpace vs, DOMImplementation di, File dest){
        ...
        if (di!=null){
            ...
            svgDoc.appendChild(svgDoc.createComment(" Generated by zvtm-svg (Zoomable Visual Transformation Machine) v" + Messages.VERSION + " http://zvtm.sourceforge.net"));
	...
	private Element createEllipse(VEllipse e){
		...
		shape.setAttribute(SVGReader._cx,String.valueOf(e.vx));
		shape.setAttribute(SVGReader._cy,String.valueOf(-e.vy));
	...
	private Element createCircle(VCircle c){
		...
		shape.setAttribute(SVGReader._cx,String.valueOf(c.vx));
		shape.setAttribute(SVGReader._cy,String.valueOf(-c.vy));
	...
	private Element createRect(VRectangle r){
		...
		if (r.getOrient()==0){
			...
			shape.setAttribute(SVGReader._x,String.valueOf(r.vx-r.getWidth()/2d));
			shape.setAttribute(SVGReader._y,String.valueOf(-r.vy-r.getHeight()/2d));
			...
			xcoords[0]=(x2*Math.cos(Math.PI-r.getOrient())+y1*Math.sin(Math.PI-r.getOrient()))+r.vx;
			xcoords[1]=(x1*Math.cos(Math.PI-r.getOrient())+y1*Math.sin(Math.PI-r.getOrient()))+r.vx;
			xcoords[2]=(x1*Math.cos(Math.PI-r.getOrient())+y2*Math.sin(Math.PI-r.getOrient()))+r.vx;
			xcoords[3]=(x2*Math.cos(Math.PI-r.getOrient())+y2*Math.sin(Math.PI-r.getOrient()))+r.vx;
			ycoords[0]=-(y1*Math.cos(Math.PI-r.getOrient())-x2*Math.sin(Math.PI-r.getOrient()))+r.vy;
			ycoords[1]=-(y1*Math.cos(Math.PI-r.getOrient())-x1*Math.sin(Math.PI-r.getOrient()))+r.vy;
			ycoords[2]=-(y2*Math.cos(Math.PI-r.getOrient())-x1*Math.sin(Math.PI-r.getOrient()))+r.vy;
			ycoords[3]=-(y2*Math.cos(Math.PI-r.getOrient())-x2*Math.sin(Math.PI-r.getOrient()))+r.vy;
		...
		while (!pi.isDone()){
			...
			switch (type){
				...
				case java.awt.geom.PathIterator.SEG_QUADTO:{
					...
					lastOp='Q';
					coords.append((seg[0])+" "+(seg[1])+" "+(seg[2])+" "+(seg[3]));
					break;
				}
				case java.awt.geom.PathIterator.SEG_CUBICTO:{
					if (lastOp!='C'){coords.append('C');} else {coords.append(' ');}
					lastOp='C';
					coords.append((seg[0])+" "+(seg[1])+" "+(seg[2])+" "+(seg[3])+" "+(seg[4])+" "+(seg[5]));
					break;
				}
			}
			pi.next();
	...
	private Element createText(VText t){
		...
		text.setAttribute(SVGReader._x,String.valueOf(t.vx));
		text.setAttribute(SVGReader._y,String.valueOf(-t.vy));
	...
	private Element createPoint(VPoint p){
		...
		shape.setAttribute(SVGReader._x,String.valueOf(p.vx));
		shape.setAttribute(SVGReader._y,String.valueOf(-p.vy));
	...
	private Element createLine(VSegment s){
		...
		shape.setAttribute("x1", String.valueOf(endPoints[0].x));
		shape.setAttribute("y1", String.valueOf(-endPoints[0].y));
		shape.setAttribute("x2", String.valueOf(endPoints[1].x));
		shape.setAttribute("y2", String.valueOf(-endPoints[1].y));
		Color c=s.getColor();
		shape.setAttribute(SVGReader._style,"stroke:rgb("+c.getRed()+","+c.getGreen()+","+c.getBlue()+")");
		if (s.getStroke()!=null){
	...
	private Element createPolygon(VShape s){
		...
		for (int j=0;j<vertices.length;j++){
			xcoords[j]=s.vx+s.getSize()/2d*Math.cos(vertexAngle)*vertices[j];
			ycoords[j]=-s.vy-s.getSize()/2d*Math.sin(vertexAngle)*vertices[j];
	...
	private Element createPolygon(VPolygon p){
		...
		double[] ycoords=new double[vertices.length];
		for (int j=0;j<vertices.length;j++){
			xcoords[j]=p.vx+vertices[j].x;
			ycoords[j]=-p.vy-vertices[j].y;
		}
		String coords="";
		for (int j=0;j<vertices.length-1;j++){
			coords+=String.valueOf(xcoords[j])+","+String.valueOf(ycoords[j])+" ";
	...
	private Element createImage(VImage i){
		...
		try {
			...
			shape.setAttribute(SVGReader._x,String.valueOf(i.vx-i.getWidth()/2d));
			shape.setAttribute(SVGReader._y,String.valueOf(-i.vy-i.getHeight()/2d));
