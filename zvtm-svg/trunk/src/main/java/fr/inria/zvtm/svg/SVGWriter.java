		...
		while (!pi.isDone()){
			...
			switch (type){
				case java.awt.geom.PathIterator.SEG_MOVETO:{
					...
					lastOp='M';
					coords.append((seg[0]+farWest)+" "+(seg[1]+farNorth));
					break;
				}
				...
				case java.awt.geom.PathIterator.SEG_QUADTO:{
					...
					lastOp='Q';
					coords.append((seg[0]+farWest)+" "+(seg[1]+farNorth)+" "+(seg[2]+farWest)+" "+(seg[3]+farNorth));
					break;
				}
				case java.awt.geom.PathIterator.SEG_CUBICTO:{
					if (lastOp!='C'){coords.append('C');} else {coords.append(' ');}
					lastOp='C';
					coords.append((seg[0]+farWest)+" "+(seg[1]+farNorth)+" "+(seg[2]+farWest)+" "+(seg[3]+farNorth)+" "+(seg[4]+farWest)+" "+(seg[5]+farNorth));
					break;
				}
			}
			pi.next();
