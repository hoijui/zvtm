...
public class DragItem extends DragableItem {
	...
	public void release1(ViewPanel v, int mod, int jpxx, int jpyy, MouseEvent e) {
		...
		if(parent.parentFrame==null){
			if(parent.viewer.getCursorHandler()!=null)parent.viewer.getCursorHandler().move(jpxx, jpyy, e);
			else ((GlyphEventDispatcherForMenu)parent.ged).getCursor().moveCursorTo(vx, vy, jpxx, jpyy);
