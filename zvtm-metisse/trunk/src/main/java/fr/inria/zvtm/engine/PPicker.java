...
public class PPicker extends Picker{
	...
	/** Will compute picking list on all glyphs of the virtual space*/
	private boolean offScreenMode = true;
	...
	public Vector<DPath> getIntersectingPaths(Camera c, int tolerance, double x, double y){
		...
		Vector<Glyph> glyphs= null;
		if(!offScreenMode) glyphs= c.getOwningSpace().getDrawnGlyphs(c.getIndex());
		else glyphs = c.getOwningSpace().getAllGlyphs();
		Object glyph;
	...
	void resetPickedGlyphsList(VirtualSpace vs, int camIndex){
		...
		if(!offScreenMode){
			...
			Vector<Glyph> gl = vs.getAllGlyphs();
			for (int i=0;i<gl.size();i++){
				try {
					gl.get(i).resetMouseIn(camIndex);
				}
				catch (NullPointerException ex){
					if (VirtualSpaceManager.debugModeON()){
						System.err.println("Recovered from error when resetting list of glyphs under mouse");
